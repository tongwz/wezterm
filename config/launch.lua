...
elseif platform.is_mac then
    ...
    options.launch_menu = {
        ...
        -- { label = 'TestHost1', args = {'env', 'WEZTERM_TITLE=TestHost1', 'ssh', 'root@120.26.121.166' } },
...
-- 监听面板焦点变化事件
wezterm.on('pane-focus-changed', function(palette, pane)
    -- 获取当前终端的命令行
    local foreground_info = pane:get_foreground_process_info()
    wezterm.log_info("myCustomFunction 被调用，testVar 的值为:", tab);
    if foreground_info and foreground_info.command_line then
        local cmd = foreground_info.command_line
        if cmd:match("^ssh") then
            -- 提取主机名
            ...
            if hostname then
                ...
                if label then
                    ...
                    pane:tab():set_title(hostname) -- 如果没有匹配的 label，使用主机名
                end
                end
        end
    end
